...
export function createAuthApi({ baseUrl, fetchImpl = fetch, tokenStorage }: AuthApiOptions) {
  ...
  return {
    ...
    async createReauthenticationChallenge(
      provider: SocialProvider,
    ): Promise<{ state: string; nonce: string; expiresAt: string }> {
      const response = await authorizedRequest(`/social/reauthenticate/challenge?provider=${provider}`, 'POST');
      if (!response) throw new Error('reauthentication_required');
      if (response.status !== 201) throw await socialError(response);
      const payload = await response.json() as Record<string, unknown>;
      if (
        typeof payload.state !== 'string' || !/^[A-Za-z0-9_-]{40,128}$/.test(payload.state) ||
    ...
    async exchangeReauthenticationIdentity(
      provider: SocialProvider,
      state: string,
      idToken: string,
    ): Promise<string> {
      const response = await authorizedRequest('/social/reauthenticate/exchange', 'POST', {
      ...
      const payload = await response.json() as Record<string, unknown>;
      if (
        Object.keys(payload).some((key) => key !== 'proof' && key !== 'expiresAt') ||
        typeof payload.proof !== 'string' || !/^reauth_[A-Za-z0-9_-]{40,}$/.test(payload.proof) ||
        typeof payload.expiresAt !== 'string' || !Number.isFinite(Date.parse(payload.expiresAt))
      ) throw new Error('auth_response_invalid');
    ...
    async deleteAccount(reauthenticationProof?: string): Promise<void> {
      const response = await authorizedRequest('/account', 'DELETE', {
        headers: reauthenticationProof ? { 'X-SCIO-Reauthentication': reauthenticationProof } : undefined,
      });
